...
class ImageConverter:
    ...
    def process_image(self, undistorted, warped_binary):
        ...
        predict = drawing(undistorted, warped_binary, curvature_params[1], curvature_params[3], self.M_inv,
                          ld_point_idx)

        return predict, front_steering_angle, center_curverad

    def pipeline(self, img, r_thresh=(200, 255), s_thresh=(128, 255), sx_thresh=(100, 150)):
        ...
        abs_sobelx = np.absolute(cv2.Sobel(l_channel, cv2.CV_64F, 1, 0, ksize=5))
    ...
    def image_callback(self, ros_image):
        ...
        # BGR, birdview image
        warped = cv2.warpPerspective(undistorted, self.M, (undistorted.shape[1], undistorted.shape[0]),
        ...
        cv2.putText(pub_image, 'Lane Curvature: %.3f m' % center_curverad, (60, 130), font, 2, (0, 255, 0), 2,
                    cv2.LINE_AA)
        cv2.putText(pub_image, 'Front Steering Angle: %.3f degrees' % (self.f_steer_curr), (60, 200), \
                    font, 2, (0, 255, 0), 2, cv2.LINE_AA)

        # Publish predicted front steering angle 
